...
function generateWarnings(data: any): string[] {
  ...
  // High risk warnings
  if (data.is_proxy === "1") {
    warnings.push("Proxy contract - Code can be changed");
  }
  if (data.is_mintable === "1") {
    warnings.push("Mintable - New tokens can be created");
  }
  if (data.can_take_back_ownership === "1") {
    warnings.push("Ownership can be reclaimed");
  }
  ...
  if (data.is_open_source !== "1") {
    warnings.push("Contract is not verified/open source");
  }
  
  // Holder concentration
  if (data.holder_count && parseInt(data.holder_count) < 100) {
    warnings.push(`Low holder count: ${data.holder_count}`);
  }
  
  // LP info
  if (data.lp_holder_count && parseInt(data.lp_holder_count) < 5) {
    warnings.push(`Very few LP holders: ${data.lp_holder_count}`);
  }
...
export function assessPoolRisk(pool: {
  ...
  if (pool.txns24hTotal < 10) {
    warnings.push("Very low trading activity (<10 txns/day)");
    riskBoost += 10;
  }
