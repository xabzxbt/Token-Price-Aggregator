...
export async function GET(req: Request) {
  try {
    ...
    const allDexPoolsRaw = [...dexPools, ...coingeckoDexPools]
      .filter((pool) => pool.liquidityUsd !== null && pool.liquidityUsd >= 100)
      .sort((a, b) => b.score - a.score);

    // Group DEX pools by DEX name - keep only best pool per DEX (highest liquidity)
    const dexPoolsMap = new Map<string, DexPoolPrice>();
    allDexPoolsRaw.forEach((pool) => {
      // Create unique key by DEX + chain
      const dexKey = `${pool.dexId}-${pool.chain}`.toLowerCase();
      const existing = dexPoolsMap.get(dexKey);
      
      // Keep the pool with highest liquidity for each DEX
      if (!existing || (pool.liquidityUsd ?? 0) > (existing.liquidityUsd ?? 0)) {
        dexPoolsMap.set(dexKey, pool);
      }
    });

    const allDexPools = Array.from(dexPoolsMap.values())
      .sort((a, b) => {
        // Sort by tier first, then by liquidity
